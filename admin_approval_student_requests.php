...
$stmt = $conn -> prepare("SELECT * FROM student_reservation INNER JOIN student USING(student_number) INNER JOIN faculty USING(faculty_id) WHERE is_active = 1 and is_faculty_approved = 1 and is_admin_approved IS NULL");
$stmt->execute();
